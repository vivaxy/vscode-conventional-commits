...
const resolve_extends_1 = __importDefault(
  require('@commitlint/resolve-extends'),
);
const load_plugin_1 = __importDefault(require('./utils/load-plugin'));
const load_config_1 = require('@commitlint/load/lib/utils/load-config');
const load_parser_opts_1 = require('@commitlint/load/lib/utils/load-parser-opts');
const pick_config_1 = require('@commitlint/load/lib/utils/pick-config');
...
async function load(seed = {}, options = {}) {
  ...
  const base =
    loaded && loaded.filepath ? path_1.default.dirname(loaded.filepath) : cwd;
  // TODO: validate loaded.config against UserConfig type
  // Might amount to breaking changes, defer until 9.0.0
  // Merge passed config with file based options
  const config = pick_config_1.pickConfig(
    merge_1.default({}, loaded ? loaded.config : null, seed),
  );
  ...
  if (typeof config.parserPreset === 'string') {
    const resolvedParserPreset = resolve_from_1.default(
      base,
      config.parserPreset,
    );
  ...
  const preset = pick_config_1.pickConfig(
    mergeWith_1.default(extended, config, w),
  );
  preset.plugins = {};
  ...
  if (typeof preset.parserPreset === 'object') {
    preset.parserPreset.parserOpts = await load_parser_opts_1.loadParserOpts(
  ...
  const qualifiedRules = (
    await Promise.all(
      Object.entries(rules || {}).map((entry) => execute_rule_1.default(entry)),
    )
